        ...
        >
            ...
            <div className="p-6">
                <div className="flex items-center mb-5">
                    <Image src={event.author.profileImage} alt={event.author.name} width={40} height={40} className="w-10 h-10 rounded-full object-cover" />
                    <div className="ml-3">
                        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{event.author.name}</p>
                ...
                <button onClick={() => onShowParticipants(event)} className="flex items-center justify-between w-full text-sm text-gray-600 dark:text-gray-400 mb-6 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50">
                    <div className="flex items-center">
                        ...
                        <span className="font-medium">{event.participants.length} {event.participants.length > 1 ? 'participants' : 'participant'}</span>
                    </div>
                    <div className="flex -space-x-3">
                        ...
                        {event.participants.length > 4 && <div className="w-7 h-7 bg-gray-200 text-gray-600 text-xs font-bold rounded-full border-2 border-white dark:border-gray-800 flex items-center justify-center">+{event.participants.length - 4}</div>}
                    </div>
                </button>
